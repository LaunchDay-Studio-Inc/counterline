...
class PrepPackViewModel @Inject constructor(
    ...
    /** Return the cheat sheet text ready for sharing via Android Share sheet. */
    fun getExportableCheatSheet(): String {
        val state = _uiState.value
        if (state.cheatSheet.isBlank()) return ""

        return buildString {
            appendLine(state.cheatSheet)
            appendLine("───────────────────────────────")
            appendLine("Generated by CounterLine")
            appendLine("https://github.com/official-stockfish/Stockfish")
            appendLine("GPL-3.0 • Engine-tested repertoire")
        }
    }

    /** Mark the export as triggered (for UI feedback). */
    fun markExportReady() {
        _uiState.update { it.copy(exportReady = true) }
    }

    /** Reset the export flag. */
    fun clearExportReady() {
        _uiState.update { it.copy(exportReady = false) }
    }
}
